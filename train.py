...
test_name='class_image/test_jidan.txt'):
        ...
        for dir,folder,file in os.walk(rootdata):
            ...
            for i in range(0,int(len(file_pic)*train_ratio)):
                dir1=dir.split('/')
                len1=len(classnames)
            ...
            for i in range(int(len(file_pic) * train_ratio),len(file_pic)):
                dir1=dir.split('/')
                len1=len(classnames)
                if dir1[-1]in classnames.keys():
                        # print(dir1[-1])
                    ...
                    classnames[dir1[-1]]=len1
                test_data = os.path.join(dir, file_pic[i]) + '\t' +str(classnames[dir1[-1]])+'\n'
